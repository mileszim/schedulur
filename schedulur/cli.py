...
class CLI:
            ...
            if os.path.exists(config_file):
                with open(config_file, 'r') as f:
                    ...
                    if 'current_user_id' in config:
                        self.current_user = self.user_service.get_user(config['current_user_id'])
    ...
    def setup_parsers(self):
        ...
        user_subparsers = user_parser.add_subparsers(dest="subcommand", help="Subcommand")
        
        # Add/create user
        add_user_parser = user_subparsers.add_parser("create", help="Create a new user profile")
        add_user_parser.add_argument("--name", required=True, help="User's full name")
        add_user_parser.add_argument("--email", required=True, help="User's email address")
        add_user_parser.add_argument("--phone", help="User's phone number")
        add_user_parser.add_argument("--zip", help="User's ZIP code")
        add_user_parser.add_argument("--insurance", help="User's insurance provider")
        add_user_parser.add_argument("--insurance-id", help="User's insurance ID")
        ...
        # Set current user
        set_user_parser = user_subparsers.add_parser("select", help="Select a user profile")
        set_user_parser.add_argument("user_id", help="User ID to select")
        
        # Set availability
        availability_parser = user_subparsers.add_parser("availability", help="Set user availability")
        availability_parser.add_argument("--day", type=int, choices=range(7), required=True, 
                                      help="Day of week (0=Monday, 6=Sunday)")
        availability_parser.add_argument("--start", required=True, help="Start time (HH:MM)")
        availability_parser.add_argument("--end", required=True, help="End time (HH:MM)")
        ...
        # Connect calendar
        connect_parser = calendar_subparsers.add_parser("connect", help="Connect to calendar")
        connect_parser.add_argument("--provider", choices=["google", "outlook", "mock"], required=True, 
                                  help="Calendar provider")
        
        # Show calendar slots
        slots_parser = calendar_subparsers.add_parser("slots", help="Show available calendar slots")
        slots_parser.add_argument("--days", type=int, default=7, help="Number of days to look ahead")
        
        # Search commands
        search_parser = subparsers.add_parser("search", help="Search for doctors")
        search_subparsers = search_parser.add_subparsers(dest="subcommand", help="Subcommand")
        
        # Search by criteria
        criteria_parser = search_subparsers.add_parser("doctor", help="Search for doctors by criteria")
        criteria_parser.add_argument("--specialization", required=True, help="Doctor specialization")
        criteria_parser.add_argument("--insurance", help="Insurance provider")
        criteria_parser.add_argument("--zip", help="ZIP code")
        criteria_parser.add_argument("--distance", type=int, default=25, help="Max distance in miles")
        criteria_parser.add_argument("--urgency", type=int, choices=range(1, 6), default=1, 
                                   help="Urgency level (1-5)")
        
        # Search by query
        query_parser = search_subparsers.add_parser("query", help="Search for doctors using natural language")
        query_parser.add_argument("query", help="Natural language search query")
        ...
        schedule_parser = appointment_subparsers.add_parser("schedule", help="Schedule an appointment with approved doctors")
        schedule_parser.add_argument("--reason", required=True, help="Reason for appointment")
        schedule_parser.add_argument("--date", help="Preferred date (YYYY-MM-DD)")
        
        # Approve doctor
        approve_parser = appointment_subparsers.add_parser("approve", help="Approve a doctor for scheduling")
        approve_parser.add_argument("doctor_id", help="Doctor ID to approve")
        
        # Reject doctor
        reject_parser = appointment_subparsers.add_parser("reject", help="Reject a doctor for scheduling")
        ...
        show_parser = appointment_subparsers.add_parser("show", help="Show appointment details")
    ...
    def check_current_user(self):
        ...
        if not self.current_user:
            print("No user profile selected. Please create or select a user profile first.")
            print("Run 'schedulur user create --name \"Your Name\" --email \"your.email@example.com\"'")
            return False
        return True
    ...
    def handle_user_command(self, args):
        ...
        if args.subcommand == "create":
            ...
            print(f"Insurance: {self.current_user.insurance_provider or 'Not provided'}")
            ...
            # Update the user
            self.user_service.update_user(self.current_user.id, self.current_user)
            
            day_name = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'][day]
            print(f"Updated availability for {day_name}: {start_time} - {end_time}")
    ...
    def handle_calendar_command(self, args):
        ...
        if args.subcommand == "connect":
            ...
            if provider == "google":
                ...
                print("In a real implementation, this would open the Microsoft OAuth flow")
                print("For now, we're using a mock implementation")
    ...
    def handle_search_command(self, args):
        ...
        if args.subcommand == "doctor":
            ...
            if not zip_code:
                print("Warning: No ZIP code provided. Distance-based results may not be accurate.")
    ...
    def print_doctor_search_results(self, doctors: List[Doctor]):
        ...
        for i, doctor in enumerate(doctors, 1):
            ...
            if doctor.earliest_available_slot:
                print(f"   Earliest available: {doctor.earliest_available_slot}")
    ...
    def handle_appointment_command(self, args):
        ...
        if args.subcommand == "schedule":
            ...
            if not approved_doctors:
                print("No approved doctors found. Please search for doctors and approve them first.")
                return
            ...
            if args.date:
                try:
                    ...
                    preferred_date = datetime(int(date_parts[0]), int(date_parts[1]), int(date_parts[2]), 9, 0)
                except (ValueError, IndexError):
                    print("Invalid date format. Please use YYYY-MM-DD.")
                    return
            
            print(f"Scheduling appointments with {len(approved_doctors)} approved doctors...")
            ...
            for doctor in approved_doctors:
                ...
                if appointment:
                    print(f"Success! Appointment scheduled for {appointment.start_time.strftime('%A, %B %d at %I:%M %p')}")
                    print(f"Location: {doctor.address}, {doctor.city}, {doctor.state} {doctor.zip_code}")
                    print(f"Appointment ID: {appointment.id}")
                    print(f"Added to your {self.current_user.calendar_provider} calendar")
                    ...
                    if call_details.get('transcript'):
                        ...
                        important_lines = filtered_lines[:3] + ["..."] + filtered_lines[-3:]
            ...
            success = self.appointment_service.approve_doctor_for_scheduling(args.doctor_id, True)
            if success:
            ...
            success = self.appointment_service.approve_doctor_for_scheduling(args.doctor_id, False)
            if success:
                doctor = self.doctor_service.get_doctor(args.doctor_id)
            ...
            if args.upcoming:
                ...
                appointments = self.appointment_service.get_user_appointments(self.current_user.id)
            ...
            for appointment in appointments:
                ...
                print(f"   When: {appointment.start_time.strftime('%A, %B %d at %I:%M %p')}")
            ...
            # Show appointment details
            appointment = self.appointment_service.get_appointment(args.appointment_id)
            if not appointment:
            ...
            if doctor:
                ...
                if doctor.address:
                    ...
                    if doctor.city and doctor.state and doctor.zip_code:
                        print(f"         {doctor.city}, {doctor.state} {doctor.zip_code}")
                if doctor.phone:
                    print(f"Phone: {doctor.phone}")
            
            print(f"\nDate: {appointment.start_time.strftime('%A, %B %d, %Y')}")
            print(f"Time: {appointment.start_time.strftime('%I:%M %p')} - {appointment.end_time.strftime('%I:%M %p')}")
            print(f"Status: {appointment.status}")
